...
def get_current_borrower_market(db: _orm.Session, borrower: _schemas.Borrower):
    ajuan = get_ajuan_by_id(db=db, borrower_id=borrower.id)
...
def patch_borrower(
    ...
    if existing_borrower:
        ...
        if m.status == "diterima":
            existing_ajuan = get_ajuan_by_id(db=db, borrower_id=existing_borrower.id)
...
def get_ajuan_by_market(db: _orm.Session, ajuan_id: int):
    return db.query(_models.Ajuan).filter(_models.Ajuan.id == ajuan_id).first()


def get_borrower_by_ajuan(db: _orm.Session, borrower_id: int):
    return db.query(_models.Borrower).filter(_models.Borrower.id == borrower_id).first()
...
# def get_umkm_by_market(db: _orm.Session, ajuan_id: int):
#     existing_ajuan = get_ajuan_by_market(db=db, ajuan_id=ajuan_id)
#     existing_borrower = get_borrower_by_ajuan(
#         db=db, borrower_id=existing_ajuan.borrower_id
#     )
#     existing_umkm = get_umkm_by_borrower(db=db, borrower_id=existing_borrower.id)

#     return existing_umkm


def get_ajuan_by_ajuan_id(db: _orm.Session, ajuan_id: int):
    existing_ajuan = get_ajuan_by_market(db=db, ajuan_id=ajuan_id)

    return existing_ajuan
